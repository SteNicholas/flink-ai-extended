...
class PythonJobGenerator(JobGenerator):
    ...
    def generate(self, sub_graph: AISubGraph, resource_dir: Text = None) -> Job:
        ...
        with NamedTemporaryFile(mode='w+b', dir=resource_dir, prefix='{}_python_'.format(job.job_name),
                                delete=False) as fp:
...
class PythonJobController(JobController):
    ...
    def submit_job(self, job: Job, job_runtime_env: JobRuntimeEnv = None) -> JobHandle:
        ...
        if 'site_packages_path' in job.job_config.properties:
            script_path = '{}/ai_flow_plugins/job_plugins/python/python_run_main.py'.format(
        ...
        bash_command = ['cd', os.path.abspath(os.path.dirname(occlum_image_path)), '&&', 'occlum', 'exec',
                        python3_location, script_path, run_graph_file.replace(occlum_image_path, ''),
...
class OcclumPythonJobPluginFactory(JobPluginFactory):
