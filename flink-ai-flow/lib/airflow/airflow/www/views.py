...
class Airflow(AirflowBaseView):  # noqa: D101  pylint: disable=too-many-public-methods
            ...
            for event_meta in event_metas:
                ...
                if sender in dag.task_ids:
                    ...
                    if sender in upstream_tasks:
                        upstream_tasks[sender] = upstream_tasks[sender].union(value)
                    else:
    ...
    def graph(self, session=None):
        ...
        for t in dag.tasks:
            ...
            if t.task_id in task_instances and dt_nr_dr_data['dr_id']:
                label_name = '_'.join([dag_id, dt_nr_dr_data['dr_id'], t.task_id, task_instances[t.task_id].try_number])
                execution_label = self.aiflow_client.get_execution_label(label_name)
                logging.info(
                    'Execution label of dag ({}) run ({}) is {}.'.format(dag_id, dt_nr_dr_data['dr_id'],
                                                                         execution_label))
        ...
        for e in edges:
            if e['source_id'] == '*':
                ...
                node_edges.add((e['source_id'], e['target_id']))
        ...
        return self.render_template(
            ...
            events=send_events,
            nodes=nodes,
            edges=[{'source_id': source_id, 'target_id': target_id} for source_id, target_id in node_edges],
            show_external_log_redirect=task_log_reader.supports_external_link,
            external_log_name=external_log_name,
    ...
    def nodes(self, session=None):
        ...
        events: Dict[str, Tuple[str, str, str]] = {}
        for t in dag.tasks:
            if t.task_id in task_instances and dag_run:
                label_name = '_'.join([dag_id, dag_run.run_id, t.task_id, task_instances[t.task_id].try_number])
                execution_label = self.aiflow_client.get_execution_label(label_name)
                logging.info(
                    'Execution label of dag ({}) run ({}) is {}.'.format(dag_id, dag_run.run_id, execution_label))
                if execution_label and execution_label.value:
                    task_instances[t.task_id].update({'execution_label': execution_label.value})
            if t.task_id in task_instances and t.executor_config is not None and 'periodic_config' in t.executor_config:
                task_instances[t.task_id].update({'periodic_config': t.executor_config['periodic_config']})
        ...
        return json.dumps({'task_instances': task_instances, 'events': send_events,
                           'edges': [{'source_id': source_id, 'target_id': target_id} for source_id, target_id in
                                     node_edges]}, cls=utils_json.AirflowJsonEncoder)
...
class TaskExecutionModelView(AirflowModelView):
    ...
    datamodel = AirflowModelView.CustomSQLAInterface(TaskExecution)  # noqa # type: ignore

    class_permission_name = permissions.RESOURCE_TASK_EXECUTION
    method_permission_name = {
        'list': 'read',
        'action_clear': 'edit',
        'action_set_running': 'edit',
...
class TaskInstanceModelView(AirflowModelView):
    ...
    def task_instance_link(attr):
        ...
        task_executions_url = url_for('TaskExecutionModelView.list', _flt_0_dag_id=dag_id, _flt_1_task_id=task_id)
        return Markup(  # noqa
            """
            <span style="white-space: nowrap;">
            <a href="{url}">{task_id}</a>
            <a href="{task_executions_url}" title="Visit all corresponding task executions">
            ...
            """
        ).format(url=url, task_id=task_id, url_root=url_root, task_executions_url=task_executions_url)
    ...
    formatters_columns = {
        # 'task_executions': task_executions_formatter,
