        ...
        <div class="control-line" v-if="showNodeType">
          <div class="line-operate">
            ...
            <div class="icons">
              ...
              <i @click="toggleDagJobNode('dag')" style="font-size: 12px;" :class="{ active: activeDag === 'dag' }"
                 @dblclick.stop.prevent>Data View</i>
              <i @click="redictDagGraphView('dagJobNode')" style="font-size: 12px;"
                 :class="{ active: activeDag === 'dagJobNode' }" @dblclick.stop.prevent>Task View</i>
            </div>
                  ...
                  <p
                    ...
                    <template v-if="showNodeType">
                      <template v-if="canChoose">
                        <el-checkbox v-model="scope.data.data.isRealNode"
...
import {MAPCOLOR} from './mapSourceType'
...
export default {
  ...
  computed: {
    ...
    itemsPosed () {
      let result = this.items.map(m => {
        let siblings = this.items.filter(i => i.layer === m.layer)
        let index = siblings.findIndex(s => s.id === m.id)
        let mid = Math.max(0, (siblings.length - 1) / 2)
    ...
    _operators () {
      let result = this.itemsPosed.map((i, index) => {
        return {
          id: i.id,
          title: i.jobTypeName,
  ...
  methods: {
    ...
    redictDagGraphView (node) {
      this.activeDag = node
      let params = location.href.split('#')[0].split('/')
      location.href = 'http://localhost:8080/tree?dag_id=' + params[params.length - 2] + '.' + params[params.length - 1]
    },
    nodeTypeChange (val, data) {
      let nodeObj = this.items.find(p => p.id === data.id) || {}
      this.$set(nodeObj, 'isRealNode', val)
      // 设置相关节点，规则：
      // 当计算节点设置为实节点后，子依赖的所有节点均设置为实节点
      // 当计算节点设置为虚节点后，父依赖的所有节点均设置为虚节点
      let nodeList = this.getRelationNode(data, val ? 'children' : 'parent')
      nodeList.forEach(v => {
    ...
    setZoom (s) {
      this.$nextTick(_ => {
        ...
        let flowchart = this.$refs.flowchart.$el
