...
def init_ai_flow_client(server_uri: str):
    global client
    client = AIFlowClient(server_uri=server_uri)
...
def data_view(project: str, workflow: str):
    ...
    workflow_meta = client.get_workflow_by_name(project_name=project, workflow_name=workflow)
...
def main(argv):
    ...
    ai_flow_server_uri = ''
    try:
        opts, args = getopt.getopt(argv, "hp:s:", ["port=", "server_uri="])
    except getopt.GetoptError:
        print('usage: web_server.py -p <port> -s <server_uri>')
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print('usage: web_server.py -p <port> -s <server_uri>')
            ...
            web_server_port = arg
        elif opt in ("-s", "--server_uri"):
            ai_flow_server_uri = arg
    init_ai_flow_client(ai_flow_server_uri)
    app.run(host='127.0.0.1', port=web_server_port)
